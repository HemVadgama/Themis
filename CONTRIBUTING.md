...
Good first contributions include a protocol plus deterministic tests, a documented linear scenario, a metric derived from existing state, an artifact-based visualizer, or an input adapter that does not bypass validation. Campaign changes must preserve actor-local information, deterministic equal-time ordering, independent random streams, fresh per-run protocol state, and explicit timeout/resource-release behavior.

Use `docs/protocols.md` for protocol work and `docs/configuration.md` when changing the schema. Artifact changes must update packaged JSON Schemas and compatibility tests without silently changing older benchmark output. Update example smoke tests, changelog, citation metadata, and user documentation with public-interface changes. Report bugs or modeling concerns with the relevant issue form; include a resolved config and Themis version when possible.
