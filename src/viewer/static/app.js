...
function renderInspector() {
  ...
  const related = [event, ...event.related_event_ids.map(id => currentRun.events.find(item => item.event_id === id)).filter(Boolean)]
    ...
    .filter(item => ["PROTOCOL_DECISION", "MANEUVER_PROPOSED", "MANEUVER_VALIDATED", "MANEUVER_REJECTED", "MANEUVER_EXECUTED", "MANEUVER_FAILED", "TRAJECTORY_REPROPAGATED", "RISK_REASSESSED", "CONJUNCTION_RESOLVED", "RISK_CREATED", "RISK_UPDATED", "RISK_CLOSED", "RISK_EXPIRED", "AUCTION_CREATED", "AUCTION_BID_CREATED", "AUCTION_BID_RECEIVED", "AUCTION_WINNER_SELECTED", "AUCTION_AWARD_SENT", "AUCTION_AWARD_RECEIVED", "AUCTION_TIMED_OUT", "AUCTION_NO_VALID_BID", "MESSAGE_SENT", "MESSAGE_DELIVERED", "MESSAGE_DROPPED", "MESSAGE_DELAYED_BEYOND_USEFULNESS"].includes(item.event_type));
